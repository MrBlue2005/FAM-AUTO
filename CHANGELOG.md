...
- Baseline/reload capture now retains bounded hidden/detached subtree metadata and source-parent relationships for the shared exact-body descent instead of filtering unsafe blocks before classification. Hidden/detached body branches remain ineligible for success, unsafe body duplicates preserve split ambiguity, and unsafe control-only siblings remain ignorable. Existing 16-root, 64-raw-node, 24-reduced-block, and helper bounds are unchanged; the earlier detached-race plausibility limitation remains explicit, and exact-body, clean-zero/newness, acknowledgement, click/reload, retry, and outcome semantics are otherwise unchanged.
