...
- Managed USER campaign creation now atomically grants exactly one visibility relation to the authenticated creator through a creation-only, service-role-only RPC. The creator identity comes from the signed server session; client recipient fields are ignored, pre-existing campaigns cannot be self-assigned, exact request replay cannot duplicate visibility, and visibility failure rolls back creation. ADMIN ACL management, unrelated-user isolation, historical campaigns, and target/device assignment semantics are unchanged.

- Added bounded, privacy-safe `captureTargetCandidates()` stage diagnostics for root discovery/cap and eligibility counts, retained candidate ordinals, root-reader parity, raw descendant ordinal windows, reduced-block signal/parent loss, and normalization-sensitive booleans. Baseline and reload share the schema, and reload now retains discovery completeness/cap context in the existing protected structural summary under the unchanged 32 KiB/four-summary reservation. No selector, cap, normalization, timing, verifier, trusted-newness, click/reload, retry, or outcome semantics changed.

- Body descent admission now accepts either truthful candidate-root containment or immutable-body containment in the already-captured bounded descendant reduction. Root parity remains unchanged and separately reported; pre-click baseline and post-reload verification share the same admission semantics and unchanged strict descent terminal rules. This addresses the detached-race false negative only when the bounded capture retains evidence. Hidden/detached, comment/reply, independent-article, interactive, substring, duplicate/split, and beyond-budget cases remain fail closed; selectors, capture/descent bounds, exact equality, canonical roots, clean-zero/newness, acknowledgement, click/reload, retry, and outcome semantics are unchanged.

- Baseline/reload capture now retains bounded hidden/detached subtree metadata and source-parent relationships for the shared exact-body descent instead of filtering unsafe blocks before classification. Hidden/detached body branches remain ineligible for success, unsafe body duplicates preserve split ambiguity, and unsafe control-only siblings remain ignorable. Existing 16-root, 64-raw-node, 24-reduced-block, and helper bounds are unchanged; exact-body, clean-zero/newness, acknowledgement, click/reload, retry, and outcome semantics are otherwise unchanged.
