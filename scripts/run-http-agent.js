...
const service = new CloudAgentService({ transport, registry, runtimeProfiles, executeTask: createCloudFacebookTaskExecutor(registry, runtimeProfiles), intervalMs: Number(process.env.RX_AGENT_HEARTBEAT_INTERVAL_MS || 30000), events: (type, data) => console.log(`RX_AGENT_EVENT:${JSON.stringify({ type, ...data })}`) });
