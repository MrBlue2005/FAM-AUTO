...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (mediaMatch && request.method === 'POST') {
      ...
      const result = await Promise.all(snapshot.sort((a: any,b: any) => a.ordinal-b.ordinal).map(async (item: any) => { const row = media.find((entry: any) => entry.media_id === item.media_id); if (!row || row.sha256 !== item.sha256 || Number(row.byte_size) !== Number(item.byte_size) || row.mime_type !== item.mime_type) throw new Error('MEDIA_SNAPSHOT_MISMATCH'); const { data, error: signError } = await supabase.storage.from(row.bucket).createSignedUrl(row.object_key, 120); if (signError) throw signError; return { media_id: row.media_id, sha256: row.sha256, byte_size: row.byte_size, mime_type: row.mime_type, ordinal: item.ordinal, download_url: data.signedUrl }; }));
