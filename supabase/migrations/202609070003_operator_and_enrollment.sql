...
declare v_enrollment_id uuid; begin
 if p_expires_at<=now() or p_expires_at>now()+interval '24 hours' then raise exception 'INVALID_ENROLLMENT_EXPIRY'; end if;
 insert into agent_enrollment_tokens(token_hash,created_by,expires_at) values(extensions.digest(p_token,'sha256'),p_created_by,p_expires_at) returning enrollment_id into v_enrollment_id;
 perform rx_cp_event('ENROLLMENT_TOKEN_CREATED',null,null,jsonb_build_object('enrollment_id',v_enrollment_id,'created_by',p_created_by));
 return jsonb_build_object('enrollment_id',v_enrollment_id,'expires_at',p_expires_at); end $$;
