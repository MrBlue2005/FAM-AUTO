...
test('managed-user migration and hosted ADMIN Users UI keep hashes and passwords server-only', () => {
  ...
  assert.match(sidebar, /id: 'users'/); assert.match(page, /if \(!isAdmin\)/); assert.match(page, /type=\{showCreatePassword \? 'text' : 'password'\}/); assert.match(page, /Adaugă utilizator/); assert.match(page, /Resetează parola/); for (const forbidden of ['localStorage', 'sessionStorage', 'indexedDB', 'document.cookie', 'console.', 'password_scrypt']) assert.equal(page.includes(forbidden), false);
});
