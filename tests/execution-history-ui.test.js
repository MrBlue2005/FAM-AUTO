...
test('execution history UI is admin-hosted, filterable, and contains no task mutation controls', async () => {
  ...
  assert.match(page, /api\.getCloudTasks/); assert.match(page, /api\.getCloudTask/); assert.match(page, /profilesForDevice/); assert.match(source('dashboard-v2', 'src', 'services', 'executionHistory.js'), /OUTCOME_UNKNOWN/);
