...
async function runCampaign(page, item, groups, campaignDay, options = {}) {
  ...
  console.log(`Grupuri de parcurs acum: ${eligibleGroups.length}`);

  emitEvent(
    'info',
    `Pornesc ${itemName}: ${eligibleGroups.length} grupuri de parcurs.`
  );
  ...
  emitState({
    ...
    etaTotal: null,
    pauseRequested: false,
    lastMessage: `Pornesc campania: ${itemName}`,
  });
  ...
  for (const group of eligibleGroups) {
    ...
    if (
      ...
      hasProcessed(history, item.id, group.id, campaignDay, facebookProfileId)
    ) {
