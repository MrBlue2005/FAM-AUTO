...
function buildQueuePlan({ config, properties, jobs, groups, history = [], now = new Date() }) {
  ...
  const groupsPostedToday = config.skipGroupsPostedToday
    ? getGroupsPostedOnDate(history, now)
    : new Set();
  ...
  const tasks = campaigns.flatMap((campaign) => {
    ...
    const eligibleGroups = applyQueueOptions(availableGroups, config);
  ...
  return {
    ...
    summary: {
      ...
      skippedToday: skippedTodayGroupIds.size,
    },
  };
}
...
function buildPreflightReport({ config, properties, jobs, groups, history = [], now = new Date() }) {
  const queuePlan = buildQueuePlan({ config, properties, jobs, groups, history, now });
  ...
  if (queuePlan.activeTasks.length === 0) {
    issues.push({
      level: 'error',
      code: queuePlan.summary.skippedToday > 0 ? 'NO_ELIGIBLE_GROUPS_TODAY' : 'EMPTY_QUEUE',
      message: queuePlan.summary.skippedToday > 0
        ? `Nu mai exista taskuri pentru azi: ${queuePlan.summary.skippedToday} grupuri au deja o postare.`
        : 'Nu exista taskuri pending sau marcate pentru retry.',
    });
  }
