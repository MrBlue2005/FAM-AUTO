...
const POST_SUBMIT_SUCCESS_PREDICATES = new Set(['BOTH_PREDICATES_PASSED', 'STORY_ID_PROOF_PASSED', 'TARGET_RELOAD_PROOF_PASSED', 'NOT_SATISFIED']);
...
const POST_SUBMIT_EVIDENCE_SOURCES = new Set(['EXPLICIT_ACKNOWLEDGEMENT', 'STORY_ID_VISIBLE_EXACT', 'STORY_ID_PENDING', 'CANONICAL_TARGET_RELOAD', 'PENDING_MODERATION_ACKNOWLEDGEMENT', 'ERROR_OR_REJECTION_ACKNOWLEDGEMENT', 'SUBMIT_TRANSPORT_FAILURE', 'NO_AUTHORITATIVE_EVIDENCE']);
const POST_SUBMIT_VERIFICATION_SURFACES = new Set(['ACKNOWLEDGEMENT_SURFACES', 'ACKNOWLEDGEMENT_AND_STORY_ID', 'ACKNOWLEDGEMENT_AND_CANONICAL_TARGET_RELOAD', 'ACKNOWLEDGEMENT_STORY_ID_AND_CANONICAL_TARGET_RELOAD']);
const STORY_VERIFICATION_RESULTS = new Set(['VISIBLE_EXACT', 'VISIBLE_MISMATCH', 'PENDING', 'NOT_FOUND', 'INACCESSIBLE', 'UNSUPPORTED', 'AMBIGUOUS']);
...
const TRANSPORT_CORRELATIONS = new Set(['PRIMARY_CREATE_RESPONSE', 'PROVEN_SHARED_ID', 'PROVEN_REQUEST_CHAIN', 'LIKELY_TEMPORAL_ONLY', 'UNPROVEN']);
...
function sanitizeTransportRequest(value = {}) {
  return {
    ...
    requestCorrelationId: /^REQ_[1-8]$/.test(value.requestCorrelationId || '') ? value.requestCorrelationId : null,
  };
}
...
function sanitizeProtectedResponseSummary(value) {
  if (!value || typeof value !== 'object') return null;
  const response = sanitizeTransportResponse(value);
  return {
    responseIndex: Math.min(7, boundedInteger(value.responseIndex) || 0),
    timestamp: response.timestamp,
    relativeToClickMs: response.relativeToClickMs,
    requestCorrelationId: response.requestCorrelationId,
    operationName: response.operationName,
    documentId: response.documentId,
    status: response.status,
    statusClass: response.statusClass,
    responseClassification: response.responseClassification,
    graphqlErrorsPresent: response.graphqlErrorsPresent,
...
function sanitizeSubmitTransport(value = {}) {
  return {
    ...
    consoleErrors: Array.isArray(value.consoleErrors) ? value.consoleErrors.slice(0, 8).map((item) => ({ timestamp: safeTransportTimestamp(item.timestamp), relativeToClickMs: safeTransportRelative(item.relativeToClickMs), sourceClass: TRANSPORT_SOURCE_CLASSES.has(item.sourceClass) ? item.sourceClass : 'UNCLASSIFIED', category: TRANSPORT_CONSOLE_CATEGORIES.has(item.category) ? item.category : 'UNKNOWN' })) : [],
...
function sanitizePostSubmitVerification(value = {}) {
  ...
  return {
    ...
    storyVerificationResult: STORY_VERIFICATION_RESULTS.has(value.storyVerificationResult) ? value.storyVerificationResult : null,
    storyTargetMatched: bool('storyTargetMatched'), storyTokenMatched: bool('storyTokenMatched'), storyBodyMatched: bool('storyBodyMatched'),
