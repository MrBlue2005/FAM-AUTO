...
Remaining Phase 4B-B work is the local synthetic Storage E2E: signed upload → object verification → `STAGED` → `READY` → authorized preview → negative/security tests. It must not cut over the dashboard or Local Agent and must keep all Supabase service-role credentials server-only.
