...
Last updated: 2026-09-08 (hostable same-origin BFF/auth foundation added)
...
1. Task A of the future hosted dashboard is complete locally at this checkpoint: `server/hosted-bff.js` plus Vercel `api/[...path].js` provide a stateless same-origin `/api/*` BFF/auth seam. It uses a signed 12-hour HttpOnly/SameSite session cookie, session-bound CSRF, exact mutation-Origin validation, and only server-side application Supabase credentials. Required production server secrets are `RX_BFF_SESSION_SIGNING_SECRET`, `RX_BFF_PUBLIC_ORIGIN`, `ADMIN_PASSWORD_SCRYPT`, `RX_APP_SUPABASE_URL`, and `RX_APP_SUPABASE_SERVICE_ROLE_KEY`; do not expose them to browser variables. The current dashboard remains on the local API, and scheduler/Facebook/Chromium/Copywriter remain local. Next scope should wire reviewed dashboard read models deliberately, not deploy or cut over implicitly.
