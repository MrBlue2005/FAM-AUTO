...
Last updated: 2026-09-08 (narrow cloud media upload/finalization added)
...
1. Task C2 of the future hosted dashboard is complete locally at this checkpoint: cloud uploads require both `VITE_CLOUD_MEDIA_UPLOAD_ENABLED=true` and server-only `RX_BFF_CLOUD_MEDIA_UPLOAD_ENABLED=true`; general cloud mutations remain blocked. A dashboard file hashes locally, receives a scoped BFF authorization, PUTs directly to private Storage, then server-finalizes independent size/SHA verification before append-only ordered post attachment via the existing RPC. Failed uploads remain STAGED and unattached. Local Multer/`app/uploads` behavior is unchanged. No deployment, import, scheduler/Facebook/Chromium/Local Agent change, or broad dashboard cutover occurred. Next work should be an explicitly scoped cloud editing/campaign-save path only if authorized.
