...
Last updated: 2026-09-08 (feature-gated cloud dashboard reads added)
...
1. Task B of the future hosted dashboard is complete locally at this checkpoint: `VITE_DASHBOARD_DATA_MODE` defaults to `LOCAL`; explicit `CLOUD_READ_ONLY` sends only supported dashboard reads through the authenticated same-origin `/api/cloud-read/*` BFF adapter. It maps application campaigns/posts, targets, folders, schedules/ordered relationships, preview text, and safe media metadata to legacy-shaped DTOs without raw DB internals or secrets. Cloud-mode mutations are rejected before any local fallback. Runtime/queue/robot/history/runs/reports/exports and scheduler execution remain unavailable in cloud mode; scheduler, Facebook/Chromium, and Copywriter remain local. No dashboard production cutover or deployment occurred. The next scope is a separately reviewed read-only UI availability treatment or Task C media preview/upload boundary, not mutation wiring.
