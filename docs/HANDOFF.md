...
Pre-click baseline and post-reload verification use the same diagnostic schema. Reload summaries now preserve discovery completeness and candidate-cap state inside the existing protected structural summary; no fifth critical summary was added, and the fixed 32 KiB/four-summary reservation still drops secondary candidate detail before aggregate capture evidence. This is observation-only: selectors, 16/64/24 caps, normalization, timing, descendant admission, exact-body classification, trusted `0 -> 1` newness, acknowledgement, click/reload limits, retry policy, and `OUTCOME_UNKNOWN` semantics are unchanged.

## G5.7FJ bounded descendant-signal admission

Candidate-root text containment is no longer the sole gate for strict body descent. A visible, attached candidate is now admitted when either its truthful root parity contains the immutable body or the already-captured 24-block descendant reduction does; root and descendant evidence remain separate safe booleans and a fixed admission-source enum. Pre-click baseline and post-reload verification use the same rule and the unchanged shared descent helper. A retained visible exact descendant can therefore reach the existing exact-body classifier, while hidden/detached, comment/reply, independent-article, interactive, split, and substring-only evidence still fails closed. No root parity is fabricated, no selector or capture bound changed, and exact equality, canonical-root validation, clean-zero policy, trusted `0 -> 1` newness, acknowledgement, composer-hidden, one-click/one-reload, no-retry, and `OUTCOME_UNKNOWN` semantics are unchanged. The detached-race false negative is addressed only when bounded descendant capture retains the signal; evidence beyond the bounded reduction remains fail closed.

## G5.7FC bounded hidden/detached capture parity

Pre-click baseline and post-reload candidate capture now retain up to the existing 24-block reduced budget of hidden or detached subtree metadata instead of discarding those blocks before shared unique-body descent. Snapshot `visible`, `attached`, `hidden`, `detached`, and bounded source-parent indices remain available to the unchanged helper: a sole unsafe body branch fails at `HIDDEN_OR_DETACHED_BOUNDARY`, visible plus unsafe body duplicates fail as `BODY_SIGNAL_SPLIT_AMBIGUOUS`, and unsafe control-only siblings still do not become body branches. Candidate-root discovery, the 64-node raw budget, reduced 24-block budget, helper depth/child/node bounds, exact-body equality, clean-zero admission, trusted `0 -> 1` newness, acknowledgement, one-click/one-reload, no-retry, and `OUTCOME_UNKNOWN` semantics are unchanged. G5.7FJ subsequently permits this retained bounded descendant signal to admit descent even when every candidate-level reader lost it; if the bounded reduction also lost or excluded the signal, the case remains fail closed.
...
## G5.7CO post-publication structural-source diagnostics
...
Phase G5.5B adds the additive `hosted_user_campaign_visibility` allowlist. ADMIN campaign visibility remains global; a managed USER sees only enabled rows assigned to its immutable `managedUserId`. The BFF applies that same database-side scope to campaign reads, previews, preflight sources, controlled execution, and live execution source resolution—browser input cannot widen it. A disabled assignment blocks future reads/source resolution but does not alter owner-scoped historical task visibility. Legacy environment USER sessions have no managed-user ACL identity and therefore receive no allowlisted campaign data. Targets remain globally listed and independently audited; no target ACL was introduced. The G5.5 rehearsal has not run.
...
- Phase D2 connects the existing ADMIN Devices-page explicit `deviceId` + `profileId` selection to the hosted campaign-preflight request. The browser submits only reviewed identifiers and optional revision guards; the BFF revalidates device status/freshness, READY ownership, and server-builds the immutable `PREFLIGHT` snapshot with `publishEnabled=false`. Safe status/result DTOs are displayed, safe rejection codes are translated for the UI, stale selections require explicit reselection, and no device/profile fallback exists. USER cannot see or invoke this routed preflight.

- Phase B4 adds hosted cloud-managed USER accounts. The environment-backed `ADMIN_USERNAME`/`ADMIN_PASSWORD_SCRYPT` remains the root/bootstrap administrator; managed USER password hashes and session versions live only in the private server-role Supabase table. ADMIN can create, enable/disable, and reset USER passwords through the hosted Admin Users surface. Disable and reset advance the session version, invalidating managed sessions on their next request. Hard deletion is deferred. The optional environment-backed `USER_USERNAME`/`USER_PASSWORD_SCRYPT` login remains a temporary compatibility path and should be retired after managed-user migration; it is unrelated to operator and Local Agent credentials.

- B4.2 lets ADMIN generate a strong browser-cryptographic password and copy one-time username/password handoff details immediately after USER creation or reset. Plaintext remains only in transient Users-page memory and is cleared on dismissal/reload; it cannot be recovered later, so Reset Password is required if credentials are lost.
...
The target-scoped reload verifier obtains a read-only baseline after final canonical-target/readiness checks and before `ATTEMPT_STARTED`. A clean zero is now positive evidence: discovery must be complete and uncapped, contain at least one visible/attached eligible post surface, and contain no immutable-body signal that remains structurally untrusted. Empty discovery, capped or incomplete discovery, candidate-evaluation errors, nested ambiguity, and visible unresolved immutable-body signals all fail closed before marker and click. Only a same-target `0 -> 1` transition of visible, attached, structurally trusted exact immutable-body post candidates may establish target-reload newness. The retained composer, comments/replies, dialogs/drafts, hidden/detached surfaces, and substring-only content cannot satisfy the baseline or reload proof. A unique fixture token remains operationally required but is never itself proof. Existing acknowledgement success, one-click/no-retry semantics, cleanup, and the fixed 32 KiB four-summary retention model remain unchanged.

### G5.7EL interactive-boundary diagnostics

The existing protected post-candidate body-subtree summary now records bounded, privacy-safe evidence for body-bearing same-post wrappers that also contain downstream interactive controls. It distinguishes isolated non-interactive body regions from mixed, interactive-node, interactive-ancestor, and ambiguous structures without retaining raw text or DOM identifiers. This is diagnostic-only: `REJECT_INTERACTIVE`, extraction, baseline admission, target reload, click, retry, and outcome semantics are unchanged.
...
- `CLOUD_CAMPAIGN_LOCAL_AGENT_PREFLIGHT = PASS`: Preview task `campaign_preflight_76809b91-b746-4057-95e4-71279b72f9e6` completed once (`TASK_CLAIMED -> TASK_RUNNING -> TASK_COMPLETED`) with `OUTCOME_UNKNOWN=0`, a zero-media verified result, and `publishEnabled=false`. Facebook, Chromium, and operational state were untouched. The normal dashboard now exposes campaign preflight rather than the validation-only synthetic `DRY_RUN` control; the underlying gated route and boolean-only admin availability check remain useful regression/target-health seams. The synthetic campaign `CLOUD_PREFLIGHT_SYNTH_20260909` and target `CLOUD_PREFLIGHT_TARGET_20260909` remain isolated hosted cleanup records pending a dedicated, revision-guarded removal. Next milestone: `CLOUD_CAMPAIGN_REAL_MEDIA_PREFLIGHT`.
