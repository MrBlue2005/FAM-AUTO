...
- `CLOUD_CAMPAIGN_LOCAL_AGENT_PREFLIGHT` is locally implemented: the restricted hosted remote-task route constructs an immutable `CAMPAIGN_PREFLIGHT` snapshot from cloud campaign/post/target rows and READY media metadata, then routes only to the configured synthetic agent/profile. Its Local Agent executor validates the profile and already materialized media with publishing hard-disabled; it never launches Chromium or contacts Facebook. Hosted validation remains separately gated and must use only isolated synthetic application data.
