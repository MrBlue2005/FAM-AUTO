...
Last updated: 2026-09-08 (CLOUD_READ_ONLY signed media preview added)
...
1. Task C1 of the future hosted dashboard is complete locally at this checkpoint: `CLOUD_READ_ONLY` now resolves READY post-linked media via authenticated `/api/cloud-read/media/:mediaId/preview` authorizations. The BFF returns only a short-lived `{ url, expiresIn }`; the dashboard uses an in-memory TTL cache and refreshes once after expiry/load failure, never persisting signed URLs. STAGED, absent, and unlinked media are denied. Local `/uploads/...` behavior is unchanged. Cloud mode visibly disables media upload/delete/cleanup and centrally rejects unsupported local-owned capabilities without fallback. Scheduler, Facebook/Chromium, and Copywriter remain local; no deployment or application mutation occurred. Next scope is Task C2 upload/finalization only after explicit review.
