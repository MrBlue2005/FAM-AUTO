...
`POST /v1/agent/tasks/:taskId/media-manifest` is an agent-authenticated Protocol v1 extension. It accepts only the current `CLAIMED` task lease, requires every requested media ID to be in immutable `tasks.payload.media`, requires READY application metadata to equal the snapshot hash/size/MIME identity, and returns only 120-second signed download URLs plus verification fields. Before `RUNNING`, the agent streams every item to a task-only temporary directory, verifies byte size and SHA-256, then derives an execution-only local-path payload without mutating the stored task snapshot. Download attempts are bounded; one failed signed-URL materialization may request one fresh manifest, never an unbounded refresh loop. Zero-media tasks bypass manifest/materialization. Signed URLs and agent/server credentials never reach the executor or Chromium. A manifest/download failure is pre-side-effect `FAILED`, not `OUTCOME_UNKNOWN`; executor failures also clean materialized media in `finally`, and `OUTCOME_UNKNOWN` remains reserved for genuine post-executor ambiguity. Task paths and media IDs are validated, sibling task directories remain isolated, and temporary data is removed on every terminal path.
