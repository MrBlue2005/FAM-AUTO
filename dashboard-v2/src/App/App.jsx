...
function DashboardApp({ auth }) {
  ...
  function renderPage() {
    ...
    if (activePage === 'executions') return <Executions isAdmin={auth.isAdmin} isManagedUser={auth.user?.managedUser === true} canControlledExecute={auth.user?.canControlledExecute === true} canLiveExecute={auth.user?.canLiveExecute === true} />;
