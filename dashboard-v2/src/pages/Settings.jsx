...
export default function Settings() {
  ...
  const [profileSetupBusyId, setProfileSetupBusyId] = useState(null);
  ...
  async function startProfileSetup(profile) {
    setProfileSetupBusyId(profile.id);
    setMessage('');

    try {
      const savedConfig = await api.saveRuntimeConfig(config);
      const savedProfile = (savedConfig.facebookProfiles || []).find(
        (item) => item.id === profile.id
      );

      if (!savedProfile) {
        throw new Error(`Profilul ${profile.label || profile.id} nu a putut fi salvat.`);
      }

      setConfig({ ...defaultConfig, ...savedConfig });
      const result = await api.setupFacebookProfile(savedProfile.id);
      setSetupProfileId(savedProfile.id);
      setMessage(result.message || `Setup pornit pentru ${savedProfile.label || savedProfile.id}.`);
    } catch (error) {
      setSetupProfileId(null);
      setMessage(error.message || `Chromium nu a putut fi deschis pentru ${profile.label || profile.id}.`);
    } finally {
      setProfileSetupBusyId(null);
    }
  }
  ...
  async function finishProfileSetup(profile) {
    ...
    try {
      ...
      setMessage(error.message || `Profilul ${profile.label || profile.id} nu a putut fi finalizat.`);
    } finally {
      setProfileSetupBusyId(null);
    }
  }
              ...
              {facebookProfiles.map((profile, index) => (
                <div className="settings-edit-row" key={profile.id}>
                  ...
                  <button
                    ...
                    disabled={profileSetupBusyId !== null}
                    onClick={() => startProfileSetup(profile)}
                  >
                    {profileSetupBusyId === profile.id ? 'Se deschide...' : 'Set profile'}
                  </button>
                  <button
                    className="primary-button small-button"
                    disabled={setupProfileId !== profile.id || profileSetupBusyId !== null}
                    onClick={() => finishProfileSetup(profile)}
                  >
                    {setupProfileId === profile.id && profileSetupBusyId === profile.id
                      ? 'Se salveaza...'
                      : 'Am terminat'}
                  </button>
