...
export default function Executions({ isAdmin = false, isManagedUser = false }) {
  ...
  const [executionTargets, setExecutionTargets] = useState([]); const [preflight, setPreflight] = useState({ deviceId: '', profileId: '', kind: 'property', campaignId: '', day: '', targetId: '' }); const [preflightMessage, setPreflightMessage] = useState('');
  ...
  useEffect(() => { if (!isManagedUser) return undefined; let ignore = false; api.getMyExecutionTargets().then((result) => { if (!ignore) setExecutionTargets(result.targets || []); }).catch((loadError) => { if (!ignore) setPreflightMessage(loadError.message || 'Țintele autorizate nu au putut fi încărcate.'); }); return () => { ignore = true; }; }, [isManagedUser]);
  ...
  async function requestPreflight() { try { const result = await api.createCampaignPreflightTask({ ...preflight, day: Number(preflight.day) }); setPreflightMessage(`Preflight solicitat: ${result.task.status}`); await load(); } catch (requestError) { setPreflightMessage(requestError.message || 'Preflight-ul nu a putut fi solicitat.'); } }
  ...
  return <div className="management-page"><header className="management-header"><div><span className="hero-eyebrow">HOSTED CONTROL PLANE</span><h1>{title}</h1><p>{description}</p></div><button className="ghost-button" onClick={load} disabled={loading}><RefreshCw size={16} /> Actualizează</button></header>
    ...
    {isManagedUser && <section className="editor-panel"><h2>Preflight autorizat</h2><p className="muted-text">Alege exclusiv o țintă atribuită de administrator. Publicarea rămâne dezactivată.</p><div className="form-grid"><label>Țintă autorizată<select value={`${preflight.deviceId}|${preflight.profileId}`} onChange={(event) => { const target = executionTargets.find((item) => `${item.deviceId}|${item.profileId}` === event.target.value); setPreflight({ ...preflight, deviceId: target?.deviceId || '', profileId: target?.profileId || '' }); }}><option value="|">Selectează</option>{executionTargets.map((target) => <option key={`${target.deviceId}|${target.profileId}`} value={`${target.deviceId}|${target.profileId}`} disabled={!target.canRequestPreflight}>{target.deviceDisplayName} · {target.profileDisplayName} · {target.canRequestPreflight ? 'disponibil' : target.online ? 'profil indisponibil' : 'offline'}</option>)}</select></label><label>Campanie ID<input value={preflight.campaignId} onChange={(event) => setPreflight({ ...preflight, campaignId: event.target.value })} /></label><label>Zi<input type="number" min="1" value={preflight.day} onChange={(event) => setPreflight({ ...preflight, day: event.target.value })} /></label><label>Target ID<input value={preflight.targetId} onChange={(event) => setPreflight({ ...preflight, targetId: event.target.value })} /></label></div><button className="secondary-button" disabled={!preflight.deviceId || !preflight.profileId || !preflight.campaignId || !preflight.day || !preflight.targetId} onClick={requestPreflight}>Rulează preflight</button>{preflightMessage && <p className="save-message">{preflightMessage}</p>}</section>}
