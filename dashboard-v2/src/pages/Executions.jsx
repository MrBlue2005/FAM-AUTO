...
import { EXECUTION_STATUS, executionStatusView, failureMessage, historyCards, profilesForDevice } from '../services/executionHistory';

function date(value) { return value ? new Intl.DateTimeFormat('ro-RO', { dateStyle: 'short', timeStyle: 'short' }).format(new Date(value)) : '—'; }

export default function Executions({ isAdmin = false }) {
  ...
  const load = async () => { setLoading(true); try { const [deviceResult, taskResult] = await Promise.all([api.getDevices(), api.getCloudTasks({ ...filters, limit: 25 })]); setDevices(deviceResult.devices || []); setTasks(taskResult.tasks || []); setError(''); } catch (loadError) { setError(loadError.message || 'Istoricul nu a putut fi încărcat.'); } finally { setLoading(false); } };
  useEffect(() => {
    let ignore = false;
    Promise.all([api.getDevices(), api.getCloudTasks({ deviceId, profileId, status, limit: 25 })])
      .then(([deviceResult, taskResult]) => { if (!ignore) { setDevices(deviceResult.devices || []); setTasks(taskResult.tasks || []); setError(''); } })
  ...
  if (!isAdmin) return <div className="management-page"><section className="editor-panel"><h1>Acces restricționat</h1><p>Execuțiile sunt disponibile numai administratorilor.</p></section></div>;
  return <div className="management-page"><header className="management-header"><div><span className="hero-eyebrow">HOSTED CONTROL PLANE</span><h1>Execuții</h1><p>Istoric cloud sigur, multi-dispozitiv. Jurnalele locale detaliate rămân în Local Studio.</p></div><button className="ghost-button" onClick={load} disabled={loading}><RefreshCw size={16} /> Actualizează</button></header>
    <section className="form-grid">{[['În rulare', cards.running], ['În așteptare', cards.queued], ['Finalizate', cards.completed], ['Eșuate / verificare', cards.failed], ['Dispozitive online', cards.onlineDevices]].map(([label, value]) => <article className="editor-panel" key={label}><strong>{value}</strong><p className="muted-text">{label}</p></article>)}</section>
    <section className="editor-panel"><div className="form-grid"><label>Dispozitiv<select value={filters.deviceId} onChange={(event) => setFilters({ deviceId: event.target.value, profileId: '', status: filters.status })}><option value="">Toate dispozitivele</option>{devices.map((device) => <option value={device.deviceId} key={device.deviceId}>{device.displayName}</option>)}</select></label><label>Profil<select value={filters.profileId} disabled={!filters.deviceId} onChange={(event) => setFilters({ ...filters, profileId: event.target.value })}><option value="">Toate profilurile</option>{profiles.map((profile) => <option value={profile.profileId} key={profile.profileId}>{profile.displayName}</option>)}</select></label><label>Status<select value={filters.status} onChange={(event) => setFilters({ ...filters, status: event.target.value })}>{Object.values(EXECUTION_STATUS).map((status) => <option value={status} key={status}>{status === 'ALL' ? 'Toate stările' : status}</option>)}</select></label></div></section>
