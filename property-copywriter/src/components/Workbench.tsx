...
export function Workbench({ demo }: { demo: boolean }) {
  ...
  return (
    <main className="shell py-10 md:py-16">
      ...
      {property && <div className="grid gap-6"><PropertyForm value={property} onChange={setProperty} /><GeneratorOptions value={options} onChange={setOptions} />
        <div className="flex flex-wrap justify-end gap-3">
          <button className={demo ? "button secondary min-w-56" : "button min-w-56"} disabled={busy !== null || manualBusy} onClick={() => generate()}>
            {busy === "all" && <span className="spinner" />} {demo ? "Generează local (demo)" : "Generează cu OpenAI API"}
          </button>
